...
export default class FirstPersonControls {
  ...
  getControlFactor() {
    return (
      ...
      (this.player.onFloor
        ? this.params.groundControlFactor
        : this.params.airControlFactor)
    );
  }
  ...
  updateVelocity(controlFactor) {
    this.player.velocity.add(
      this.player.direction.multiplyScalar(
        controlFactor * this.params.movespeedFactor
      )
    );
  }
  ...
  moveCollider() {
    const deltaPosition = this.player.velocity
      .clone()
      .multiplyScalar(this.engine.time.delta);
    ...
    if (result) {
      if (!this.player.onFloor) {
        this.player.velocity.addScaledVector(
          result.normal,
          -result.normal.dot(this.player.velocity)
        );
      }
   ...
   */
  shift() {
    this.params.movespeedFactor = this.keyStates.get("BUTTON_ADS_DOWN")
      ? 2.5
      : 3.5;
  }
