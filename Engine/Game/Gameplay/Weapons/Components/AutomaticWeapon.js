...
export default class AutomaticWeapon extends BaseWeapon {
  ...
  initializeInterpolants() {
    const positions = new Float32Array(this.magazineSize).map(
      (_, i) => i * this.fireRate
    );
  ...
  fire() {
    ...
    const floatTypedArray0 = this.bulletPositionInterpolant.evaluate(
      this.recoverLine
    );
    ...
    const floatTypedArray1 = this.bulletPositionDeltaInterpolant.evaluate(
      this.recoverLine
    );
